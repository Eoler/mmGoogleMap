...
/* mm_widget_googlemap
 * @version 0.9.0
 * @description Shows an interactive Google Map with two-way geolocation sync
 ...
 global $modx, $mm_fields, $modx_lang_attribute;
  ...
  if ($evt->name == 'OnDocFormRender' && useThisRule($roles, $templates)) {
    ...
    $output .= "      var elmfield = \$j('#{$fieldName}');
      ...
      var elmsection = \$j('<div class=\"sectionHeader\">' + elmparent.find('.warning').text() + '</div><div class=\"sectionBody tmplvars\"><div class=\"mmwidgetGoogleMap_{$fieldName}\" style=\"position:relative; width:{$mapw}; height:{$maph};\"></div></div>');
    ...
    function loadGmapscript(){
      ...
      script.src = '//maps.google.com/maps/api/js?v=3&sensor=false&callback=pagemapsloaded&language={$modx_lang_attribute}';
